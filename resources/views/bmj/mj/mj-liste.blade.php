...
@section('content')
<!-- @include('search') -->
<h2  class="container-fluid d-flex ">MODIFIER MISE A JOUR</h2>
    ...
    <nav aria-label="breadcrumb " class="first  d-md-flex" >
         <ol class="breadcrumb indigo lighten-6 first-1 shadow-lg mb-5  ">
            ...
            <li ><a class="black-text active-2  " href="#">
                <span >BMJ\Modifier mise à jour</span></a><i class="fas fa-solid fa-chevron-right ml-md-3 ml-1"></i> </li>
            ...
            <tbody>
                @foreach ($Listes as $Liste )
                    <tr class="table-row" data-href="@if ($modifier ==1)
                                {{route('edit.mj',['table'=>json_decode(json_encode($Liste->id),true)])}}
                            @else
                                {{route('create.mj',['table'=>json_decode(json_encode($Liste->id),true)])}}
                            @endif
                            ">
