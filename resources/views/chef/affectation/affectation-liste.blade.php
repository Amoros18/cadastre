...
@if ($modifier==1)MODIFIER NUMERO DE DOSSIER
@else
            ...
            <tbody>
                @foreach ($Listes as $Liste )
                    <tr class="table-row" data-href="
                            @if ($modifier ==1)
                                {{route('edit.affectation',['table'=>json_decode(json_encode($Liste->id),true)])}}
                            @else
                                {{route('create.affectation',['table'=>json_decode(json_encode($Liste->id),true)])}}
                            @endif
                            ">
