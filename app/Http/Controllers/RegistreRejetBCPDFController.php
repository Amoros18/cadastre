...
class RegistreRejetBCPDFController extends Controller
{
    ...
    function index(Request $request){
        ...
        $recherche = $table->join('rejet_controlles','nouveau_dossiers.numero_dossier','=','rejet_controlles.numero_dossier')
                            ->select('nouveau_dossiers.*','rejet_controlles.*')->get();
        $registre = $recherche;
        return view('registres.rejet-controle')->with([
            'registre'=>$registre,
    ...
    function convert_registre_to_html($registre){
        $output = '
        <h3 align="center"> Registre de Rejet Controle <h3>
